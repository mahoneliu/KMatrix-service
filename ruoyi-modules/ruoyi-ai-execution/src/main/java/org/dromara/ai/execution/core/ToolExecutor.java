 ...
 * 支持两种执行方式：
 * <ul>
 *   <li>{@link #execute} - 返回纯文本（旧版兼容）</li>
 *   <li>{@link #executeForResult} - 返回 {@link ToolResult}（支持富媒体）</li>
 * </ul>
 * 默认 {@code executeForResult} 会调用 {@code execute} 并包装为 {@link ToolResult#ofText}。
 * 如需返回图片等富媒体，只需覆盖 {@code executeForResult} 方法即可。
 *
 * @author Mahone
 * @date 2026-03-20
 */
