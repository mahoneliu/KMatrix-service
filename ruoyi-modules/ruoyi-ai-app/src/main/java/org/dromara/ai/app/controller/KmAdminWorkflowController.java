...
public class KmAdminWorkflowController extends BaseController {
    ...
    public SseEmitter execute(@RequestBody Map<String, Object> params) {
        ...
        try {
            ...
            new Thread(() -> {
                try {
                    WorkflowExecutionReq req = WorkflowExecutionReq.builder().appId(appVo.getAppId()).dslData(appVo.getDslData()).enableExecutionDetail(appVo.getEnableExecutionDetail()).message(chatSendBo.getMessage()).sessionId(-1L).userId(userId).build();
