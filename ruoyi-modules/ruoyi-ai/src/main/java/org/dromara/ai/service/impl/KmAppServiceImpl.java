...
public class KmAppServiceImpl implements IKmAppService {
    ...
    public String insertByBo(KmAppBo bo) {
        ...
        if ("2".equals(add.getAppType()) && StringUtils.isNotBlank(add.getDslData())) {
            WorkflowConfig config = JsonUtils.parseObject(add.getDslData(), WorkflowConfig.class);
            config.validate();
        }
    ...
    public Boolean updateByBo(KmAppBo bo) {
        ...
        // 如果是工作流类型应用,校验工作流配置
        if ("2".equals(app.getAppType()) && StringUtils.isNotBlank(update.getDslData())) {
            WorkflowConfig config = JsonUtils.parseObject(update.getDslData(), WorkflowConfig.class);
            config.validate();
        }
