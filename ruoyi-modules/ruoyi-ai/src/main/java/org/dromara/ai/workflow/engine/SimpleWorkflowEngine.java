...
// * 执行所有节点
// */
// private String executeNodes(WorkflowConfig config, WorkflowState
// chatWorkflowState,
...
// // 解析输入参数
// Map<String, Object> inputs = resolveInputs(nodeConfig.getInputs(),
// nodeOutputs, context);

// // 设置节点配置
// context.setNodeConfig(nodeConfig.getConfig() != null ? nodeConfig.getConfig()
// : new HashMap<>());
...
// // 更新节点执行记录
// instanceService.updateNodeExecution(executionId,
// NodeExecutionStatus.COMPLETED, output.getOutputs(),
// nodeName, duration);
...
// // 发送节点完成事件（仅发送节点名称）
// sendSseEvent(emitter, SseEventType.NODE_COMPLETE, Map.of("nodeName",
// nodeName));
...
// // 计算执行耗时
// duration = System.currentTimeMillis() - startTime;
// instanceService.updateNodeExecution(executionId, NodeExecutionStatus.FAILED,
// errorOutput, nodeName,
// duration);
...
// * 查找节点配置
// */
// private WorkflowConfig.NodeConfig findNodeConfig(WorkflowConfig config,
// String nodeId) {
...
// * 解析表达式
// */
// private Object resolveExpression(String expression, Map<String, NodeOutput>
// nodeOutputs,
// NodeContext context) {
// // 去掉 ${ 和 }
...
// * 计算条件表达式
// */
// private boolean evaluateCondition(String condition, NodeOutput output,
// NodeContext context) {
...
// * 发送SSE事件
// */
// private void sendSseEvent(SseEmitter emitter, SseEventType eventType,
