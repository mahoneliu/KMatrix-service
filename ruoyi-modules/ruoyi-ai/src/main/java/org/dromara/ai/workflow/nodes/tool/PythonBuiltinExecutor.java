...
public class PythonBuiltinExecutor implements ToolExecutor {
    ...
    public String execute(String arguments) throws Exception {
        ...
        try {
            ...
            // 将参数 JSON 写入临时文件，路径通过 sys.argv[1] 传递给脚本
            // 脚本约定：import json, sys; args = json.load(open(sys.argv[1]))
            ...
            // 立即关闭 Python 的标准输入流（EOF），防止某些环境下触发交互模式阻塞
            process.getOutputStream().close();

            // Allow 30 seconds for execution
            ...
            try {
                // On Windows, the process might still hold handles for a short time
                FileUtil.del(tempDir);
            } catch (Exception e) {
                log.warn("清理 Python 工具临时目录失败: {}, 目录: {}", e.getMessage(), tempDir.getAbsolutePath());
                // Don't throw, we already have the result or an error from the process
            }
        }
    }
}
