 ...
 * 工具执行分发器
 *
 * @deprecated 已迁移至 {@link org.dromara.ai.execution.core.ToolExecutionDispatcher}
 * @author Mahone
 * @date 2026-03-20
 */
@Deprecated
...
public class ToolExecutionDispatcher {
    ...
    public static ToolResult dispatchForResult(ToolExecutionRequest request, List<ToolBinding> bindings) {
        ...
        if (bindingOpt.isEmpty()) {
            ...
            return ToolResult.ofText("Error: Tool '" + toolName + "' not found or not bound.");
        }
        ...
        try {
            ...
            log.info("工具 [{}] 执行成功, hasContents={}", toolName, result.hasContents());
            return result;
        } catch (Exception e) {
            log.error("执行工具 [{}] 时发生异常", toolName, e);
            return ToolResult.ofText("Error executing tool: " + e.getMessage());
        }
    }
