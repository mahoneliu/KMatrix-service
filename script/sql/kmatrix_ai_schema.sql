...
CREATE TABLE `km_document` (
  ...
  `dataset_id` bigint(20) NOT NULL COMMENT '所属数据集ID',
  `original_filename` varchar(255) NOT NULL COMMENT '原始文件名',
  `oss_url` varchar(500) NOT NULL COMMENT '文件OSS路径',
  ...
  `parse_error` varchar(1000) DEFAULT '' COMMENT '解析错误信息',
...
DROP TABLE IF EXISTS `km_document_chunk`;
...
(11, 'KNOWLEDGE_RETRIEVAL', '知识检索', 'mdi:book-search', '#f59e0b', 'ai', '从知识库中检索相关文档片段，用于RAG对话', '0', '1', '0', '0', '[{"key":"query","label":"查询文本","type":"string","required":true,"description":"用于检索的查询文本"}]', '[{"key":"context","label":"检索上下文","type":"string","required":true,"description":"拼接后的上下文文本"},{"key":"docCount","label":"文档数量","type":"number","required":true,"description":"检索到的文档数量"},{"key":"retrievedDocs","label":"检索结果","type":"array","required":true,"description":"检索到的文档片段列表"}]', 1, NULL, NULL, NULL, '2026-01-07 15:24:03', 1, '2026-01-31 10:28:03', NULL);
...
(2020, '工作流模板', 2000, 3, 'workflow-template', 'ai/workflow-template/index', '1', '0', 'C', '0', '0', 'ai:workflowTemplate:list', 'mdi:file-document-outline', 1, NOW(), '工作流模板管理'),
...
(2025, '模板工作流编排', 2000, 10, 'template-editor', 'ai/template-editor/index', '1', '0', 'C', '1', '0', 'ai:templateEditor:view', 'mdi:database-search', 1, NOW(), '模板工作流编排');

-- 角色权限
INSERT INTO `sys_role_menu` (`role_id`, `menu_id`) 
SELECT 1, menu_id FROM sys_menu WHERE menu_id >= 2000 AND menu_id <= 2025;
